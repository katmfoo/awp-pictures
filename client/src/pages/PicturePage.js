...
import { Form, Header, TextArea, Button, Image, Icon, Comment, Loader } from 'semantic-ui-react';
...
export default class PicturePage extends React.Component {
  ...
  async deleteComment(comment) {
    ...
    if (should_delete && !this.state.picture_loading) {
      // Set picture loading
      this.setState({picture_loading: true});
  ...
  async postComment() {
    ...
    if (should_submit && !this.state.picture_loading) {
  ...
  render() {
    return (
      <div>
        {this.state.picture_loading && 
          <div style={{marginTop: '200px'}}>
            <Loader active={this.state.picture_loading}>Loading</Loader>
          </div>
        }
        ...
        {this.state.picture.comments && !this.state.picture_loading ? (this.state.picture.comments.length === 0 ? <div style={{margin: '20px 0'}}>No comments</div> : null) : null}
        ...
        {getLoggedInUserId() && !this.state.picture_loading &&
          <div>
